...
const UploadIPFSNode = () => {
    ...
    const onDrop = (acceptedFiles) => {
        ...
        if (!acceptedFileTypes.includes(selectedFile.type)) {
            setUploadState(prev => ({ ...prev, uploadStatus: 'Invalid file type. Please upload a PDF, PNG, or JPEG file.' }));
            return;
        }

        if (selectedFile.size > maxFileSize) {
            setUploadState(prev => ({ ...prev, uploadStatus: 'File size exceeds the 100 MB limit.' }));
            return;
        }
    ...
    const handleUploadAndSave = useCallback(async () => {
        ...
        try {
            const ipfsData = await uploadToLocalIPFS(file);

            if (ipfsData && ipfsData.Hash) {
                setUploadState(prev => ({ ...prev, uploadStatus: 'File uploaded with local node. Saving transaction to Blockchain...' }));

                const signer = await connectToMetamask();
                const transaction = await signer.sendTransaction({
                    to: uploadState.account,
                    data: toUtf8Bytes(ipfsData.Hash),
                });
            ...
            if (error.code === 4001) {
                setUploadState(prev => ({ ...prev, uploadStatus: 'Transaction cancelled by user.' }));
            } else if (error.message.includes('insufficient funds')) {
                setUploadState(prev => ({ ...prev, uploadStatus: 'Insufficient funds for transaction. Please fund your wallet.' }));
            } else {
                setUploadState(prev => ({ ...prev, uploadStatus: 'An error occurred. Please try again.' }));
            }
        } finally {
            ...
            {uploadState.transactionHash && (
                <p className="transaction-hash">
                    <button
                        ...
                        onClick={async () => {
                            const ipfsUrl = `https://ipfs.io/ipfs/${uploadState.utf8String}`;
                            try {
